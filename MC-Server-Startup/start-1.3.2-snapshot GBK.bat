:: 调试
 ...
 if exist eula.txt ( set eula=true ) else ( set eula=false )
 cls
...
:Modify_MODs_PLUGINs
 ...
 :Modify_MODs
  if not exist ".\mods" ( echo [ERROR]:没有mods文件夹 & echo 正在退回... & goto Settings ) 
  cd ".\mods"
  dir
  goto un_ban
 :Modify_PLUGINs
  if not exist ".\plugins" ( echo [ERROR]:没有plugins文件夹 & echo 正在退回... & goto Settings )
  cd ".\plugins"
  dir
 :un_ban
  echo 请输入模组/插件名称:
  set /p "%mods_plugins%="
  if not exist ".\%mods_plugins%" ( if exist ".\%mods_plugins%.jar" ( set mods_plugins=%mods_plugins%.jar ) else ( if exist ".\%mods_plugins%.ban" ( ren ".\%mods_plugins%.ban" %mods_plugins% & echo 模组/插件 %mods_plugins% 已启用 ) else ( echo [ERROR]:无法处理的错误[:(] & pause >nul & exit /b ) ) )
  echo 请选择操作:{[1]禁用模组[2]启用模组}(直接输入序号)
  choice /C:12 /N
  if %errorlevel%==1 ( ren ".\%mods_plugins%" %mods_plugins%.ban & echo 模组/插件 %mods_plugins% 已禁用 )
  if %errorlevel%==2 ( ren ".\%mods_plugins%.ban" %mods_plugins% & echo 模组/插件 %mods_plugins% 已启用 )
  cd ..
  goto Settings
...
:CheckServer & :: 检测核心
 echo 自动检测核心中......
 if exist ".\fabric-server-launch.jar" set "Server=fabric-server-launch.jar" & echo 检测到核心:fabric-server-launcher.jar & goto Settings
 if exist ".\Forge.jar" set "Server=Forge.jar" & echo 检测到核心:Forge.jar & goto Settings
 if exist ".\quilt-server-launch.jar" set "Server=quilt-server-launch.jar" & echo 检测到核心:quilt-server-launcher.jar & goto Settings
...
:setServer & :: 设置核心(如果未检测到)
 ...
 set /p "Server="
 if exist ".\%Server%" ( echo 检测到核心:%Server% & goto Settings )
 echo [ERROR]:服务器核心不存在!
...
:Start & :: 开服
 ...
 echo =========================================
 java -jar -Dfile.encoding=GBK -Xms%RAMmin%M -Xmx%RAMmax%M %Server% nogui
