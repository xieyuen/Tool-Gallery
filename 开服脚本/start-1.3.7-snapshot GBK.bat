...
:Welcome & ::欢迎界面
 ...
 echo 脚本版本: 1.3.7 snapshot UTF-8
 ...
 echo 更新日志:
 echo 1 添加了Java路径选择器
 echo 2 添加Settings界面退出选项
 echo 3 添加模组/插件内添加mods/plugins文件夹的功能
 echo 4 优化了部分代码
 echo   4.1 重写了部分if语句
 echo   4.2 简化了部分代码
 echo --------------------------------------
...
:setServer & :: 设置核心(如果未检测到)
 ...
 if %Check_Server%==true (
    ...
    if %erl%==2 goto AutoCheckingServer
    if %erl%==1 goto setServer
 )
 echo [ERROR]:核心%Server%不存在!
 if %CheckServer%==true ( 
    goto CheckServer 
...
:Start & :: 开服
 ...
 echo =========================================
 powershell /C %Java% -jar -Dfile.encoding=UTF8 -Xms%RAMmin%M -Xmx%RAMmax%M %Server% nogui
 if %eula%==false goto First_Start
 set /a restart+=1
 set /a error+=1
 if %chk_mod%==1 goto restart_1
 if %chk_mod%==5 goto restart_5
 if %chk_mod%==10 goto restart_10
 if %chk_mod%==infinity goto Start

:restart_1
 if %restart%==1 goto Crash
 goto Start
:restart_5
